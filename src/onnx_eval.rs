...
use candle_onnx::onnx::{self, GraphProto};
    ...
    for node in graph.node.iter() {
        ...
        match node.op_type.as_str() {
            ...
            "MaxPool" => {
                ...
                let mut xs = get(&node.input[0])?.clone();
                if let Some(d) = pads {
                    if d.iter().any(|&v| v != 0) {
                        match *d {
                            [p1, p2, p3, p4] => {
                                xs = xs
                                    .pad_with_zeros(2, p1 as usize, p3 as usize)?
                                    .pad_with_zeros(3, p2 as usize, p4 as usize)?;
                            }
                            _ => bail!("unsupported MaxPool padding {pads:?}"),
                        }
                    }
                }
